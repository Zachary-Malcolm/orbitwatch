...
import type { Conjunction, ScreenRequest } from './conjunctions';

// Close-approach screening. It runs off the main thread in conjunctionWorker.ts; it lives in its
// own module so the unit tests can call it directly.
//
...
/** Screen one time window; `onProgress` is called every 20 steps. */
export function screen(request: ScreenRequest, onProgress?: (done: number, total: number) => void): ScreenOutput {
  const t0 = performance.now();
  const { lines, startMs, hours, thresholdKm, groups, skipIntra } = request;
  ...
  for (let k = 0; k <= steps; k++) {
    ...
    for (const [key, [others, starlink]] of grid) {
      ...
      for (const [dx, dy, dz] of HALF_NEIGHBOURS) {
        ...
        const nb = sameCell ? grid.get(key)! : grid.get(cellKey(cx + dx, cy + dy, cz + dz));
        if (!nb) continue;
        compare(others, nb[0], sameCell);
        compare(others, nb[1], false);
        if (!sameCell) compare(starlink, nb[0], false);
      }
    }
    if (k % 20 === 0) onProgress?.(k, steps);
  }
  ...
  return { events, screened: ok.reduce((sum, v) => sum + v, 0), pairsChecked, ms: performance.now() - t0 };
}
